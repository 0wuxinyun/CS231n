...
def affine_forward(x, w, b):
    ...
    stretch the input to a column vector 
    ...
    # Input: image
    '''
    Stretch the image to a single vector 
    '''
    x_stretch=np.reshape(x,(N,-1))
    z=np.dot(x_stretch,w)+b
    out=z
...
def affine_backward(dout, cache):
    ...
    x_stretch=np.reshape(x,(N,-1))
    db=np.sum(dout,0)
    dw=np.dot(x_stretch.T,dout)
    dx_stretch=np.dot(dout,w.T)
    shape_l=[N]+list(d_s)
    dx=dx_stretch.reshape(shape_l)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def relu_forward(x):
    ...
    # np.maximum : take two arguments and compute the element wise maximum
    out=np.maximum(x,0)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        var=np.var(x,axis=0)
        xu=x-mean
        sq_var_inv=np.sqrt(var+eps)
        # epsilon is used to ensure non zero devide 
        x_norm = (x-mean)/sq_var_inv
        out = gamma*x_norm+beta
        running_mean=running_mean*momentum+(1-momentum)*mean
        running_var=running_var*momentum+(1-momentum)*var

        # cache for backward pass :
        cache=(x_norm, xu, var, 1/sq_var_inv, gamma, eps) 
...
def batchnorm_backward(dout, cache):
    ...
    # ! all the gradient has same shape as the original input 
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    x=x.T
    xu=x-mean
    denomiator=np.sqrt(var+eps)
    x_norm = (x-mean)/denomiator
    x_norm=x_norm.T
    out = gamma*x_norm+beta
    cache=(xu,eps,var,x_norm,denomiator,gamma)
...
def layernorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    xu,eps,var,x_norm,denomiator,gamma= cache
    ...
    x_norm=x_norm.T
    N,D = x_norm.shape
    dx = (1./N)*1/denomiator*(N*dx_norm - np.sum(dx_norm, axis=0) - x_norm*np.sum(dx_norm*x_norm, axis=0))
    dx = dx.T
